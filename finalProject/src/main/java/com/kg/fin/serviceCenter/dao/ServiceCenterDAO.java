package com.kg.fin.serviceCenter.dao;
...
public interface ServiceCenterDAO {
	...
	//--공지사항
	List<HashMap<String, Object>> noticeListView(HashMap paramMap); //전체 게시글 불러오기
	int noticeListCnt(HashMap paramMap); //전체 게시글 수
	
	int noticeWriteContents(HashMap paramMap); //공지사항 글 작성하기
	
	HashMap<String, Object> noticeDetailView(HashMap paramMap); //공지사항 글 불러오기
	void noticeAddCnt(HashMap paramMap); //게시글 조회수 카운트
	
	int noticeDelete(HashMap paramMap); //공지사항 게시글 삭제
	
	//--묻고답하기
	List<HashMap<String, Object>> qnaListView(HashMap paramMap); //전체 게시글 불러오기
	int qnaListCnt(HashMap paramMap); //전체 게시글 수
