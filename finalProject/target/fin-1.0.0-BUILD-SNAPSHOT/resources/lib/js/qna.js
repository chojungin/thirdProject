...
$(document).ready(function() {
	...
	ajaxUtils.doPost("/serviceCenter/qnaList.do", param, qnaAllListView);
	$(document).delegate('#qnaTable tr', 'click', clickedQnaTable);  
	
});

function clickedQnaTable(){
	var tr = $(this);
	var td = tr.children();
	...
	if(check != '번호' ){
		location.href = "/serviceCenter/qnaView.do?no=" + check;
	}
}

function qnaAllListView(dataList){
	//console.log(dataList);
	$('#qnaTd1').html('');
	var keyList = [ 'SC_QNA_NO','SC_QNA_TITLE','SC_QNA_WRITER','SC_QNA_WRITE_DATE','SC_QNA_STATUS'];
	var key = '';
